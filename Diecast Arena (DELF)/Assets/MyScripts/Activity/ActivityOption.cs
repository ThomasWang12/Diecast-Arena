...
public class ActivityOption : MonoBehaviour
{
    ...
    public void ApplyOptionsLocal(int activityIndex)
    {
        ...
        {
            activityObject.GetComponent<RaceActivity>().totalLap = RaceOption(currentRaceLap);
        }

        // Collection Battle
        if (master.activityList[activityIndex].type == activityType.CollectionBattle)
        {
            activityObject.GetComponent<CollectActivity>().duration = CollectOption(currentCollectDuration);
        }

        // Car Hunt
        if (master.activityList[activityIndex].type == activityType.CarHunt)
        {
            activityObject.GetComponent<HuntActivity>().duration = HuntOption(currentHuntDuration);
        }
    }
    ...
    public void ApplyOptions(int activityIndex, int launcherOption)
    {
        ...
        if (master.activityList[activityIndex].type == activityType.CollectionBattle)
        {
            activityObject.GetComponent<CollectActivity>().duration = CollectOption(launcherOption);
        }

        // Car Hunt
        if (master.activityList[activityIndex].type == activityType.CarHunt)
        {
            activityObject.GetComponent<HuntActivity>().duration = HuntOption(launcherOption);
        }
    }

    public int RaceOption(int option)
    {
        return raceLaps[option];
    }

    public int CollectOption(int option)
    {
        return Mathf.RoundToInt(collectDurations[option] * 60);
    }

    public int HuntOption(int option)
    {
        return Mathf.RoundToInt(huntDurations[option] * 60);
    }
}
