...
public class InputManager : MonoBehaviour
{
    ...
    [HideInInspector] public bool showGamepadHint = true;
    ...
    void Update()
    {
        ...
        if (master.currentState == gameState.Menu || master.currentState == gameState.Session)
        {
            if (!UI.isToggled)
            {
                if (Input.GetKeyDown(KeyCode.Escape))
                {
                    ...
                    if (toggleQuitGame) sound.Play(Sound.name.Select);
                }

                if (toggleQuitGame && Input.GetKeyDown(KeyCode.Return))
                {
                    Application.Quit();
                }
            }
        }
        UI.msg_quitGame = toggleQuitGame;
        ...
        if (allowExitActivity)
        {
            if (!UI.isToggled)
            {
                ...
                {
                    toggleExitActivity = !toggleExitActivity;
                    if (toggleExitActivity) sound.Play(Sound.name.Select);
                }
        ...
        else toggleExitActivity = false;
        UI.msg_exitActivity = toggleExitActivity;
    }

    public void ResetToggles()
    {
        toggleQuitGame = false;
        toggleExitActivity = false;
    }
    ...
    inputType CheckInputType()
    {
        ...
        if (currentInputType == inputType.Gamepad)
        {
            if (GamepadInput())
            {
                ...
                if (master.ready) showGamepadHint = false;
            }
