...
public class GameMaster : MonoBehaviour
{
    ...
    {
        Menu,
        Session, Session_StartingActivity,
    ...
    public enum activityType { RaceDestination, RaceCircuit, CheckpointRush, CarHunt }
    [System.Serializable]
    ...
    public void StartActivity(int i)
    {
        ...
        IEnumerator ActivityStartSequence(int index, activityType type)
        {
            ...
            input.ForceBrake();
            UI.FadeBlack("Out");
            yield return new WaitForSeconds(UI.animationClips[UI.AnimNameToIndex("Black Fade")].length);
            ...
            if (type == activityType.RaceDestination || type == activityType.RaceCircuit)
                activityList[index].mainObject.GetComponent<CheckpointCircuit>().InitializeRace();
            ...
            yield return new WaitForSeconds(UI.animationClips[UI.AnimNameToIndex("Activity Countdown 321")].length);
            ...
            UI.ActivityCountdownStart();
            UI.ActivityUI(type, "Show");

            if (type == activityType.RaceDestination || type == activityType.RaceCircuit)
                activityList[index].mainObject.GetComponent<CheckpointCircuit>().StartRace();
    ...
    public void FinishActivity(int i)
    {
        ...
        IEnumerator ActivityFinishSequence(int index, activityType type)
        {
            ...
            UI.tmpReturnSession.enabled = false;
            UI.FadeBlack("Out");
            yield return new WaitForSeconds(UI.animationClips[UI.AnimNameToIndex("Black Fade")].length);

            if (type == activityType.RaceDestination || type == activityType.RaceCircuit)
                activityList[index].mainObject.GetComponent<CheckpointCircuit>().Reset();
    ...
    void OnGameStateChange(gameState currentState)
    {
        if (lastState != currentState)
        {
            if (Dev.log_gameState)
            {
                Debug.Log("Game State: " + currentState + " | Display: " + GameStateInfo(currentState));
            }

            UI.DisplayGameSate(GameStateInfo(currentState));
