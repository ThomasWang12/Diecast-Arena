...
public class GameMaster : MonoBehaviour
{
    ...
    [HideInInspector] public ActivityOption activityOption;
    ...
    public void GetReady()
    {
        ...
        if (network.localPlay)
            ActivityLog.Write(Dev.logType.LocalPlay);
        else ActivityLog.Write(Dev.logType.NetworkPlay);
    ...
    void Update()
    {
        ...
        else
        {
            try
            {
                // The line starts throwing errors when the host disconnected but client hasn't reached auto-reset time yet
                playerPos = player.transform.position;
            }
            catch { AutoReset.Relaunch(Dev.logType.DisconnectRelaunch); }
        }
        ...
        if (returnSession)
        {
            ...
            {
                // %& Local Play / Network
                if (network.localPlay) ReturnToSession(activeActivityIndex);
                else network.ExitActivityServerRpc(activeActivityIndex);
            }
        }
    }
    ...
    public void ExitActivity(int i)
    {
        ...
        ActivityLog.WriteActivityExit(Methods.GetPlayerName(), i);
    }
